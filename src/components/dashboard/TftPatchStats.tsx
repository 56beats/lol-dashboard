import type { TftPatchStat } from "@/lib/dashboard/tft";

type Props = {
  stats: TftPatchStat[];
};

export function TftPatchStats({ stats }: Props) {
  if (stats.length === 0) {
    return (
      <div className="border-border bg-surface rounded-2xl border p-5 shadow-lg backdrop-blur">
        <div className="text-muted mb-3 text-sm">パッチ別統計</div>
  ...
  return (
    <div className="border-border bg-surface rounded-2xl border p-5 shadow-lg backdrop-blur">
      ...
      <div className="space-y-3">
        {stats.map((stat) => (
          <div key={stat.patch} className="rounded-xl border border-border/60 p-3">
